...
func FindOverpaymentsModifiedSince(ctx context.Context, provider xerogolang.IProvider, session goth.Session, modifiedSince time.Time, querystringParameters map[string]string) (*Overpayments, error) {
	...
	overpaymentResponseBytes, err := provider.Find(ctx, session, "Overpayments", additionalHeaders, querystringParameters)
	if err != nil {
		return nil, err
	}
...
// If you need details then add a 'page' querystringParameter and get 100 Overpayments at a time
// additional querystringParameters such as where, page, order can be added as a map
func FindOverpayments(ctx context.Context, provider xerogolang.IProvider, session goth.Session, querystringParameters map[string]string) (*Overpayments, error) {
	return FindOverpaymentsModifiedSince(ctx, provider, session, dayZero, querystringParameters)
}

// FindOverpayment will get a single overpayment - overpaymentID can be a GUID for an overpayment or an overpayment number
func FindOverpayment(ctx context.Context, provider xerogolang.IProvider, session goth.Session, overpaymentID string) (*Overpayments, error) {
	additionalHeaders := map[string]string{
		"Accept": "application/json",
	}

	overpaymentResponseBytes, err := provider.Find(ctx, session, "Overpayments/"+overpaymentID, additionalHeaders, nil)
	if err != nil {
		return nil, err
	}
...
func (o *Overpayments) Allocate(ctx context.Context, provider xerogolang.IProvider, session goth.Session, allocations Allocations) (*Overpayments, error) {
	additionalHeaders := map[string]string{
		"Accept":       "application/json",
	...
	overpaymentResponseBytes, err := provider.Create(ctx, session, "Overpayments/"+o.Overpayments[0].OverpaymentID+"/Allocations", additionalHeaders, body)
	if err != nil {
		return nil, err
	}
